...
\section{Results}
We have not been able to run the sentiment analysis on the sample set yet (although we are making good progress on the implementation). Therefore we can only make a hypothesis about the results. We expect to find a correlation between the IMDb ratings and the general sentiment on the web. As IMDb is the most popular source for movie information, we expect their reviewers to be a fair representation of the general population. In the same sense we expect this of the Common Crawl dataset.
...
The results of the statistical tests on the relation between sentiment and IMDb rating and the relation between sentiment and sales performance are definitely going to be reported. Depending on the results of our analysis, we will determine on an additional form of result presentation. This might involve a web interface that allows users to search for sentiment distribution / IMDb rating and sales performance of a specific movie, but we are dependent on the cooperation of the IMDb licensing department before we are allowed to do so.
